...
def initialize_patient(patient_id: Union[str, int], to_tensors=False):
    ...
    params = Params(
        **{k: np.array(float(v)) for k, v in all_params.items() if k in Params._fields}
    )
    init_state = np.array([all_params[f"x0_{i}"] for i in range(1, 14)])
    unused_params = {
        k: v
        for k, v in all_params.items()
        if (k not in Params._fields) and (k[:2] != "x0")
    }
...
def generate_meals_one_day(rng, bw: float):
    ...
    time_lb = np.array([5, 9, 10, 14, 16, 20]) * 60.0
    time_ub = np.array([9, 10, 14, 16, 20, 23]) * 60.0
    time_mu = np.array([7, 9.5, 12, 15, 18, 21.5]) * 60.0
    ...
    for p, tlb, tub, tbar, tsd, mbar, msd in zip(
        ...
        if rng.rand() < p:
            ...
            amount = round(max(rng.normal(mbar, msd), 0.0))
...
def generate_insulin_events(meals, icr, num_days):
    ...
    bolus_times = meals[:, 0] - 5.0  # bolus a few minutes before meals
